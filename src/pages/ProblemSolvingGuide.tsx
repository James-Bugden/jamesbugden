import { Clock, ChevronDown, Menu, Brain, Target, Lightbulb, CheckCircle2, XCircle } from "lucide-react";
  ...
  return (
    <div className="bg-background border border-border rounded-xl p-5 md:p-6 mb-6">
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-3 bg-background border border-border rounded-lg px-4 py-2.5">
            ...
            <div className="flex gap-1.5 shrink-0">
              <button onClick={() => setScores(prev => ({ ...prev, [i]: prev[i] === true ? null : true }))} className={`px-3 py-1 rounded text-xs font-bold transition-colors ${scores[i] === true ? "bg-green-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>YES</button>
              <button onClick={() => setScores(prev => ({ ...prev, [i]: prev[i] === false ? null : false }))} className={`px-3 py-1 rounded text-xs font-bold transition-colors ${scores[i] === false ? "bg-red-500 text-white" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>NO</button>
            </div>
          </div>
        ))}
        ...
        {answered > 0 && <button onClick={() => setScores({})} className="text-xs text-muted-foreground hover:text-foreground transition-colors mt-1">Reset</button>}
      </div>
    </div>
  );
}
...
function InterviewChecklist() {
  ...
  return (
    <div className="bg-background border border-border rounded-xl p-5 md:p-6">
      ...
      {checkedCount > 0 && <button onClick={() => setChecked({})} className="text-xs text-muted-foreground hover:text-foreground transition-colors mt-3">Reset all</button>}
    </div>
  );
}
...
{/* Hero */}
      <section className="pt-28 md:pt-36 pb-14 md:pb-20 px-5 md:px-6 bg-executive-green">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <p className="text-lg md:text-xl text-cream/90 mb-3">Based on "Problem Solving 101" by Ken Watanabe</p>
        ...
        <div className="container mx-auto max-w-3xl">
          <div className="flex items-start gap-5 mb-8">
            ...
            <div className="pt-3">
              ...
              <p className="text-gold-soft/80">Keep levelling up your interview skills</p>
            </div>
          </div>
          ...
          <div className="bg-cream/5 border border-cream/10 rounded-xl p-5 md:p-6 mb-8">
            ...
            <p className="text-cream/90 text-sm italic mb-4">"Problem Solving 101: A Simple Book for Smart People" by Ken Watanabe</p>
          </div>
      ...
      <InlineRating contentId="guide_problem_solving" locale="en" />
