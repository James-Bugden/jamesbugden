...
function getBracket(score: number) {
  if (score >= 50) return { label: "Strong Candidate", color: "#05944F", summary: "You're ready — focus on targeting the right companies and negotiating well." };
  if (score >= 35) return { label: "Almost There", color: "#C9A961", summary: "You're close. A few targeted improvements will make a big difference." };
  if (score >= 20) return { label: "Building Foundation", color: "#E68A00", summary: "You have real potential. A structured plan will get you there in 2–3 months." };
  return { label: "Getting Started", color: "#C75146", summary: "Everyone starts somewhere. Start with English and resume, then build from there." };
}
  ...
  const renderIntro = () => (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#FBF7F0" }}>
      ...
      <nav className="px-5 py-4 flex items-center justify-between">
        <Link to="/" className="font-heading text-lg font-bold tracking-wide" style={{ color: "#1B3A2F" }}>
          JAMES BUGDEN
        </Link>
