          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-muted rounded-xl overflow-hidden shadow-premium print:hidden">
              ...
              <div className="p-6 space-y-6">
                <div><div className="font-semibold text-foreground mb-3">重要成就（過去 12 個月）</div><table className="w-full text-sm"><thead><tr className="border-b border-border"><th className="text-left py-2 text-muted-foreground">成就</th><th className="text-left py-2 text-muted-foreground">影響 / 數據</th></tr></thead><tbody>{exampleAchievements.map((item, i) => (<tr key={i} className="border-b border-border"><td className="py-2 text-foreground">{item.achievement}</td><td className="py-2 text-foreground">{item.impact}</td></tr>))}</tbody></table></div>
                <div><div className="font-semibold text-foreground mb-3">市場數據</div><ul className="text-sm space-y-1"><li className="text-foreground">• 我目前的薪酬：{exampleMarketData.currentComp}</li><li className="text-foreground">• 這個職位的市場範圍：{exampleMarketData.marketRange}</li><li className="text-foreground">• 來源：{exampleMarketData.source}</li><li className="text-foreground">• 差距：{exampleMarketData.gap}</li></ul></div>
                <div><div className="font-semibold text-foreground mb-3">我的要求</div><ul className="text-sm space-y-1"><li className="text-foreground">• 首要：{exampleAsk.primary}</li><li className="text-foreground">• 替代方案：{exampleAsk.alternative}</li><li className="text-foreground">• 底薪凍結時的備案：{exampleAsk.fallback}</li></ul></div>
                <div><div className="font-semibold text-foreground mb-3">對話重點</div><div className="space-y-3 text-sm">{Object.entries(exampleTalkingPoints).map(([key, val]) => (<div key={key} className="border-l-4 border-gold pl-3"><p className="text-muted-foreground font-semibold">{key === 'opening' ? '開場' : key === 'data' ? '數據' : key === 'ask' ? '要求' : '如果被拒'}：</p><p className="text-foreground italic">「{val}」</p></div>))}</div></div>
              </div>
            </div>
      ...
      <section className="pb-12 px-5 md:px-6 print:hidden">
        <div className="container mx-auto max-w-5xl">
          <div className="bg-gold/10 rounded-xl p-6 border-l-4 border-gold">
            <p className="font-heading text-lg text-gold mb-4">💡 提示</p>
