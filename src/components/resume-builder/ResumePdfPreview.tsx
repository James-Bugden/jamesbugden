...
// resumeHasCJK import removed — preview no longer short-circuits on CJK
// content. The subset font makes CJK preview viable (see ResumePDF.tsx
// CJK_FONT_SOURCES[Noto Sans TC]).
...
export const ResumePdfPreview = React.memo(function ResumePdfPreview({
  ...
  //
  // Note on CJK: the preview pipeline now uses a self-hosted MOE-4808
  // subset of Noto Sans TC (~1.1 MB WOFF2) instead of the full 1.4 MB
  // fontsource file. Smaller glyph count parses faster in fontkit —
  // previously the full font blocked the main thread 30-75s, now it
  // should complete within a few seconds. Rare characters outside the
  // MOE 4808 set render as tofu in preview but the Download path still
  // has the full font via the server-side render.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
      ...
      try {
        await Promise.race([
          (async () => {
            ...
            // The CJK TC primary source is the self-hosted subset WOFF2
            // (public/fonts/noto-sans-tc-subset-*). Fontkit parses it fast
            // enough that we no longer need skipCJK to avoid a main-thread
            // freeze. Users see actual Chinese glyphs in preview now.
