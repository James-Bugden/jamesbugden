    ...
    <Card>
      <CardContent className="p-4 md:p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">Link Click Tracking</h3>
        ...
        <div className="border rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            <span className="text-2xl font-bold">{fmt(totalClicks)}</span>
            <span className="text-sm text-muted-foreground ml-2">total clicks</span>
          </div>
        </div>
...
function DemographicsSection() {
  ...
  const fetchedAt = data[0]?.fetched_at
    ? new Date(data[0].fetched_at).toLocaleDateString()
    : "—";

  const countryData = byType("country").slice(0, 10).map(d => ({
    name: d.breakdown_value,
    value: Number(d.percentage),
  }));

  const cityData = byType("city").slice(0, 10).map(d => ({
    name: d.breakdown_value,
    value: Number(d.percentage),
  }));

  const ageData = byType("age").map(d => ({
    name: d.breakdown_value,
    value: Number(d.percentage),
  }));

  const genderData = byType("gender").map(d => ({
    name: d.breakdown_value,
    value: Number(d.percentage),
  }));
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-medium text-muted-foreground">Audience Demographics</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
