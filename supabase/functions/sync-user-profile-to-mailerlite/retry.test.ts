...
const fields = { target_role: "Engineer" };
...
Deno.test("callMailerLiteWithRetry: 200 on first attempt → ok with 1 attempt", async () => {
  ...
  // Body should include email + fields and NOT include groups (preserves group membership).
...
Deno.test("callMailerLiteWithRetry: fails fast on 4xx (not 429)", async () => {
  const { fetcher, calls } = fakeFetcher([
    { status: 422, body: { errors: { email: ["bad"] } } },
  ]);
  const result = await callMailerLiteWithRetry({
    apiKey,
    email,
